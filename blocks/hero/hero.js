...
const src1 = "https://publish-p152536-e1620746.adobeaemcloud.com/content/dam/patina/osaka/assets/home-hero-video.mp4";
  const video = document.createElement('video');
  const source = document.createElement('source');
  source.setAttribute('src', src1);
  source.setAttribute('type', `video/${src1.split('.').pop()}`);
  video.append(source);
...
export default function decorate(block) {
  ...
  const backgroundVideoSrc = "https://publish-p152536-e1620746.adobeaemcloud.com/content/dam/patina/osaka/assets/home-hero-video.mp4";
  const modalVideoSrc = config['modal-video'];
  const headingHtml = "<h1 class='text-h1'>THE RHYTHM</br>OF FREEDOM</h1>";
  const buttonLink = "ENTER THE RHYTHM";
  ...
  if (buttonLink) {
    ...
    button.innerHTML = `<span>${buttonLink.textContent}</span><div class="hero-cta-icon"></div>`;
